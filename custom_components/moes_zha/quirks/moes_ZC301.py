"""MOES ZC301 curtain motor quirk for _TZ3210_hs01bacp / TS030F."""
...
from zhaquirks.tuya import TuyaCommand
from zhaquirks.tuya.builder import TuyaQuirkBuilder
from zhaquirks.tuya.mcu import TuyaMCUCluster, TuyaWindowCovering


class MoesStandardCommandCover(TuyaWindowCovering):
    """Use standard Window Covering commands while keeping Tuya DP position reports."""

    class AttributeDefs(TuyaWindowCovering.AttributeDefs):
        motor_calibration = foundation.ZCLAttributeDef(
            id=0xF002,
            type=t.enum8,
            access="rw",
    ...
    async def write_attributes(self, attributes, *args, **kwargs):
        """Send real ZCL writes for calibration attributes."""
        return await Cluster.write_attributes(
            self,
            attributes,
            *args,
            **kwargs,
    ...
    async def command(self, command_id, *args, **kwargs):
        """Send standard Window Covering commands."""
...
(
    ...
    .tuya_cover(
        ...
        invert=False,
        cover_cfg=MoesStandardCommandCover,
    )
    .write_attr_button(
    ...
    .command_button(
        command_name="stop",
        cluster_id=WindowCovering.cluster_id,
        unique_id_suffix="calibration_completed",
        translation_key="mark_calibrated",
        fallback_name="Stop and save current limit",
    )
    .write_attr_button(
        attribute_name="motor_calibration",
        attribute_value=MoesMotorCalibration.None_,
        cluster_id=WindowCovering.cluster_id,
        unique_id_suffix="calibration_enter",
        translation_key="enter_calibration",
    ...
    .write_attr_button(
        ...
        fallback_name="Finish entire calibration",
    )
    .tuya_enum(
        dp_id=7,
